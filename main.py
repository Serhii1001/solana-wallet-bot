...
# Initialize Telegram bot and Flask
bot = telebot.TeleBot(TELEGRAM_TOKEN)
app = Flask(__name__)

# Set webhook (remove any existing)
bot.remove_webhook()
...
# Helpers for Dexscreener and Helius
def get_symbol(mint):
...
def analyze_wallet(wallet):
    ...
    for tx in txs:
        ...
        for tr in tx.get('tokenTransfers', []):
            mint   = tr['mint']
            amount = float(tr['tokenAmount']) / (10**tr['decimals'])
            direction = (

...
def analyze_wallet(wallet):
    ...
    for tx in txs:
        ...
        for tr in tx.get('tokenTransfers', []):
            ...
            if mint == 'GUM5Vd4qe5kgTBHNfmLRaZSKKmYdAWxLJ18DY1Uzpump':
                print(f"DEBUG {mint} tx {tx.get('signature')} direction={direction} spent={spent} earned={earned} fee={fee} ts={ts_dt}")
            ...
            if direction == 'buy':
                ...
                if rec['first_ts'] is None:
                    ...
                    if mint == 'GUM5Vd4qe5kgTBHNfmLRaZSKKmYdAWxLJ18DY1Uzpump':
                        print(f"DEBUG first_ts={rec['first_ts']} first_price={rec['first_price']}")
            else:
                rec['sells'] += 1
                rec['earned'] += earned
                rec['fee'] += fee
                ...
                if mint == 'GUM5Vd4qe5kgTBHNfmLRaZSKKmYdAWxLJ18DY1Uzpump':
                    print(f"DEBUG last_ts={rec['last_ts']} last_price={rec['last_price']}")
            mint = tr['mint']
            direction = ('buy' if tr.get('toUserAccount') == wallet
                         else 'sell' if tr.get('fromUserAccount') == wallet
                         else None)

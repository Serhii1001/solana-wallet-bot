...
def format_duration_seconds(seconds):
    """Преобразуем длительность в секундах в читаемый формат"""
    sec = int(seconds)
...
def analyze_wallet(wallet):
    ...
    for tx in txs:
        ...
        for tr in tx.get('tokenTransfers', []):
            mint = tr['mint']; direction = ('buy' if tr.get('toUserAccount') == wallet else 'sell' if tr.get('fromUserAccount') == wallet else None)
            if not direction: continue
            amount = tr.get('tokenAmount',0)/10**tr.get('decimals',0)
            rec = tokens.setdefault(mint, {
                'mint': mint, 'spent_sol':0.0,'earned_sol':0.0,'buys':0,'sells':0,
                'first_buy_ts':None,'last_sell_ts':None,'first_price':None,'last_price':None,
                'first_fee':0.0,'last_fee':0.0
            })
            if direction=='buy':
                rec['buys']+=1; rec['spent_sol']+=sol_spent; rec['first_fee']+=fee
                if not rec['first_buy_ts']:
                    rec['first_buy_ts']=ts_str; rec['first_price']=round(sol_spent/amount,6) if amount else 0
            else:
                rec['sells']+=1; rec['earned_sol']+=sol_earned; rec['last_fee']+=fee
                rec['last_sell_ts']=ts_str; rec['last_price']=round(sol_earned/amount,6) if amount else 0

    # Статистика
    for rec in tokens.values():
        spent, earned = rec['spent_sol'], rec['earned_sol']
        delta = earned-spent; pct = round(delta/spent*100,2) if spent else 0
        rec.update({'delta_sol':round(delta,4),'delta_pct':pct,'last_trade':rec['last_sell_ts'] or rec['first_buy_ts']})
        rec['entry_mcap']=get_mcap(rec['mint']); rec['exit_mcap']=rec['entry_mcap']; rec['current_mcap']=get_mcap(rec['mint'])
        total_spent+=spent; total_earned+=earned
        if delta>0: wins+=1; profit_percents.append(pct)
    ...
    summary={'wallet':wallet,'winrate':winrate,'pnl_r':pnl,'avg_win_pct':avg_win,'pnl_loss':round(abs(min(0,pnl)),4),
             'balance_change_pct':balance_pct,'time_period':'30 days','sol_price':f"{SOL_PRICE} $",'balance_sol':f"{current_balance} SOL"}
...
def generate_excel(tokens, summary):
    ...
    vals1=[summary['wallet'],None,None,None,f"{summary['winrate']}%",f"{summary['pnl_r']} SOL",None,
           f"{summary['avg_win_pct']}%",f"{summary['pnl_loss']} SOL",f"{summary['balance_change_pct']}%",None,
           summary['time_period'],summary['sol_price'],summary['balance_sol']]
    ws.append(headers1); ws.append(vals1)
    ws.append([None]*len(headers1)); ws.append(['Tokens entry MCAP:']); ws.append(['<5k','5k-30k','30k-100k','100k-300k','300k+']); ws.append([None]*len(headers1)); ws.append([None]*len(headers1))
    # Таблица
    hdrs=['Token','Spent SOL','Earned SOL','Delta Sol','Delta %','Buys','Sells','Last trade','Income','Outcome','Fee','Period',
          'First buy Mcap','Last tx Mcap','Current Mcap','Contract','Dexscreener','Photon']
    ws.append(hdrs)
    for c in range(1,len(hdrs)+1): cell=ws.cell(ws.max_row,c); cell.font=Font(bold=True); cell.fill=PatternFill(start_color="DDDDDD",end_color="DDDDDD",fill_type="solid")
    ...
    for rec in tokens.values():
        ...
        if rec['first_buy_ts'] and rec['last_sell_ts']:
            ...
            period='-'
        row=[rec['mint'],f"{rec['spent_sol']} SOL",f"{rec['earned_sol']} SOL",f"{rec['delta_sol']} SOL",f"{rec['delta_pct']}%",
        ...
        for col in (4,5): ws.cell(ws.max_row,col).fill=PatternFill(start_color=color,end_color=color,fill_type="solid")

    fname=f"report_{summary['wallet']}_{datetime.now().strftime('%Y%m%d%H%M%S')}.xlsx"; wb.save(fname)
    return fname

# Bot handlers остаются без изменений
@bot.message_handler(commands=['start'])
def welcome(message): bot.reply_to(message, "Привет! Пришли Solana-кошелек для отчёта.")

@bot.message_handler(func=lambda m: True)
def handle_wallet(message):
    wallet=message.text.strip(); bot.reply_to(message,"Формирую отчёт, подожди...")
    try:
        tokens,summary=analyze_wallet(wallet)
        if not tokens: return bot.send_message(message.chat.id,"Не найдено транзакций.")
...
if __name__=="__main__":
    bot.remove_webhook(); threading.Thread(target=bot.polling,daemon=True).start()
    PORT=int(os.getenv('PORT',10000)); Handler=http.server.SimpleHTTPRequestHandler
    with socketserver.TCPServer(("",PORT),Handler) as httpd: print(f"HTTP сервер на порту {PORT}"); httpd.serve_forever()
